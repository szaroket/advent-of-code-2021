    ...
    for number in drawn_numbers:
        for board in list_of_boards:
            ...
            if drawn_numbers.index(number) >= 4 and check_if_bingo(board):
                ...
                sum_of_numbers = calculate_sum_of_unmarked_fields(board)
                list_of_boards = [
                    board_card
                    for board_card in list_of_boards
                    if not (board_card == board).all()
                ]
    return sum_of_numbers, win_number
...
if __name__ == "__main__":
    ...
    unmarked_fields_sum, number = mark_number(drawn_numbers, list_of_boards)
    print(f"Sum of unmarked fields: {unmarked_fields_sum} and number: {number}")
